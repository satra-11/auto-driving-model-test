...
def run_single_model_evaluation(args: argparse.Namespace):
    ...
    with mlflow.start_run() as run:
        ...
        try:
            ...
            try:
                loaded_object = torch.load(args.model_path, map_location=device)
                if isinstance(loaded_object, torch.nn.Module):
                    print("Fallback: Loaded full model object.")
                    model = loaded_object
                else:
                    print("Fallback failed: Loaded object is not a torch.nn.Module (likely a dict or state_dict).")
                    raise e
